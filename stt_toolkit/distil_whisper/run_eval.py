...
def main():
    ...
    use_wandb = data_args.report_to == "wandb"
    ...
    if use_wandb:
        if not is_wandb_available():
            raise ValueError("Install wandb or use --report_to none")
        import wandb
    ...
    for dataset_dict in tqdm(dataset_names_dict, desc="Loading datasets..."):
        ...
        sample_counts[pretty_name] = count
    ...
    def benchmark(batch):
        if model_pipeline is None:
            ...
            # Timestamp generation is optional, but WER compares plain text.
            batch["transcription"] = [
    ...
    for split in datasets_evaluated_progress_bar:
        ...
        total_samples = sample_counts.get(split)
        for result in tqdm(
            result_iter,
            total=total_samples,
        ...
        if use_wandb and data_args.log_predictions:
            write_wandb_pred(
                wandb_logger,
                transcriptions,
                references,
    ...
    metrics_path = os.path.join(data_args.output_dir, "eval_metrics.json")
    with open(metrics_path + ".tmp", "w") as json_file:
        json.dump(dict(stats_dataset), json_file, indent=2)
        json_file.write("\n")
